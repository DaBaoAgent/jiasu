...
**一次性配置脚本**（已执行 ✅，归档于 `D:\@佳康顺\D盘脚本归档\jiasu_setup_once.ps1`，右键配套 `jiasu_setup_once.bat`；注意 .ps1 右键菜单没有"以管理员身份运行"选项，.bat 包装器才有）：
...
3. 结果写 `D:\@佳康顺\D盘脚本归档\jiasu_admin_result.txt` 供验证（含"jiasu admin done"标记）

> **✅ 状态（2026-08-14 已配置）**：`D:\@佳康顺\D盘脚本归档\jiasu_admin_result.txt` 已生成（含 `jiasu admin done`），
> UAC 自动提权/还原点/遥测禁用/HermesAutoClean/DISM 全部完成。此后所有提权操作静默执行，
> cron 每日清理的管理员子集已解锁。注：脚本最后一步 DISM 需 5-20 分钟，窗口"不动"是正常的。
